    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> CreateRole(CreateRoleViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    System.Diagnostics.Debug.WriteLine($"создана роль {model.RoleName}");

                    return RedirectToAction("Роли");
                }

                foreach (var error in result.Errors)
                {
                    System.Diagnostics.Debug.WriteLine($"Неудачная попытка создать роль {model.RoleName}");
        ...
        public async Task<IActionResult> EditRole(EditRoleViewModel model)
        {
            ...
            if (role == null)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"не удалось найти роль с id = {model.Id}");
                return NotFound();
            }
            ...
            if (result.Succeeded)
            {
                System.Diagnostics.Debug.WriteLine($"Отредактированная роль с id = {model.Id}");
            ...
            foreach (var error in result.Errors)
            {
                System.Diagnostics.Debug.WriteLine($"Не удалось отредактировать роль с id = {model.Id}");
        ...
        public async Task<IActionResult> EditUsersInRole(int roleId)
        {
            ...
            if (role == null)
            {
                ViewBag.ErrorMessage = $"Роль с Id = {roleId} не нашлось";
                System.Diagnostics.Debug.WriteLine($"Роль не найдена с id = {roleId}");
        ...
        public async Task<IActionResult> EditUsersInRole(List<UserRoleViewModel> model, int roleId)
        {
            ...
            for (int i = 0; i < model.Count; i++)
            {
                ...
                if (ifSucceeded)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("Правильно отредактированы пользователи в роли");
        ...
        /// <summary>
        /// Собственная реализация, которая проверяет, находится ли пользователь в данной роли
        /// </summary>
        ...
        /// <summary>
        /// Собственная реализация удаления пользователя с данной ролью
        /// </summary>
        /// <param name="user"></param>
        ...
        public async Task<IActionResult> EditUser(UserFormViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Правильное редактирование пользователя {user.UserName}");
        ...
        {
            // todo добавить сохранение фотографий
        ...
        public async Task<IActionResult> Stock(StockViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Создание запаса {stock.Name} для продукта с id={stock.ProductId}");
        ...
        public async Task<IActionResult> DeleteStock(int id)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"Убран запас {stock.Name} для продукта с id = {stock.ProductId}");
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> DeleteColor(int id)
        {
            ...
            await _context.SaveChangesAsync();
            System.Diagnostics.Debug.WriteLine($"Удален цвет {color.Name}");
        ...
        public async Task<IActionResult> DeleteCategory(int id)
        {
            ...
            await _context.SaveChangesAsync();
            System.Diagnostics.Debug.WriteLine($"Удаление категории {category.Name}");
        ...
        public async Task<IActionResult> DeleteBrand(int id)
        {
            ...
            await _context.SaveChangesAsync();
            System.Diagnostics.Debug.WriteLine($"Удаление бренда {brand.Name}");
        ...
        public async Task<IActionResult> SaveProduct(ProductFormViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (product.Id != 0)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Модифицированный продукт {product.Name} с id={product.Id}");
                }
                else
                {
                    await _context.Products.AddAsync(product);
                    System.Diagnostics.Debug.WriteLine($"Добавлен продукт {product.Name} с id={product.Id}");
                }
        ...
        public async Task<IActionResult> DeleteProduct(int id)
        {
            ...
            await _context.SaveChangesAsync();
            System.Diagnostics.Debug.WriteLine($"Удален продукт {productInDb.Name} с id={productInDb.Id}");
        ...
        public async Task<IActionResult> SaveBrand(Brand model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id != 0)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Модифицированный бренд с id={model.Id}");
                }
                ...
                else
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Бренд быд добавлен с id={brand.Id}");
        ...
        public async Task<IActionResult> SaveCategory(Category model)
        {
            if (ModelState.IsValid)
            {
                if (model.Id != 0)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Изменение категории с id={model.Id}");
                }
                else
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Добавлена категория с id={category.Id}");
        ...
        public async Task<IActionResult> SaveColor(Color model)
        {
            if (ModelState.IsValid)
            {
                if (model.Id != 0)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Изменение цвета с id={model.Id}");
                ...
                else
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Добавление цвета с id={model.Id}");
                }
            ...
            System.Diagnostics.Debug.WriteLine("Цвет успешно сохранен");
        ...
        /// <summary>
        /// Удаляет заказ из базы и возвращает продукты из заказа на склад
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<IActionResult> CancelOrder(int id)
        {
            ...
            //  удалите их в базе данных из orderProducts и добавьте их в продажу
            ...
            await _context.SaveChangesAsync();
            System.Diagnostics.Debug.WriteLine($"Отменен заказ с id={order.Id} для пользователя {order.User}");
        ...
        public async Task<IActionResult> SentOrder(int id)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"Отправлен заказ с id={order.Id} для пользователя {order.User}");
